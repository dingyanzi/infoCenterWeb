...
router.beforeEach(async (to, from, next) => {
  ...
  // 公共页面，任何时候都可以跳转
  if (to.path === PAGE_PATH_404 || to.path === '/screen/car' || to.path === '/Android/fieldWork') {
    next();
    return;
  }
...
export function buildRoutes(menuRouterList) {
  ...
  for (const e of menuList) {
    ...
    if (e.menuType === MENU_TYPE_ENUM.CATALOG.value || e.menuType === MENU_TYPE_ENUM.POINTS.value) {
      ...
      // 尝试多种匹配方式
      let component = modules[relativePath];
      if (!component) {
        component = modules[`${relativePath}.vue`];
      }
      if (!component) {
        component = modules[`${relativePath}/index.vue`];
      }

      if (component) {
        route.component = component;
      } else {
        console.warn(`[SmartAdmin] 路由组件加载失败: ${e.menuName}, path: ${e.path}, component: ${relativePath}`);
      }
    }
